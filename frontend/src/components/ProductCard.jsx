...
const ProductCard = ({product}) => {
    ...
    return product && (
        <div onClick={ ()=> {navigate(`/menu/${product.category.toLowerCase()}/${product._id}`);
            scrollTo(0,0)
        } } className="border border-gray-500/50 rounded-md md:px-2 p-2 bg-white max-w-80 w-80 md:w-80 hover:shadow-xl hover:scale-105 transition">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="space-y-1 items-end justify-between">
                    ...
                    <div onClick={ (e)=> { e.stopPropagation() } } className="text-black">
                        {!cart[product._id] ? (
                            <button onClick={() => addToCart(product._id)}  className="flex cursor-pointer items-center justify-center gap-1 bg-indigo-100 border border-indigo-300 md:w-[130px] w-[180px] h-[34px] rounded font-medium" >
                            ...
                            <div className="flex items-center justify-center gap-2 md:w-[130px] w-[180px] h-[34px] bg-indigo-500/25 rounded select-none">
                                <button onClick={() => {removeFromCart(product._id)}} className="cursor-pointer text-2xl px-2 h-full" >
                                    -
                                </button>
                                <span className="w-5 text-center">{cart[product._id]}</span>
                                <button onClick={() => {addToCart(product._id)}} className="cursor-pointer text-2xl px-2 h-full" >
                                    +
                                </button>
                            </div>
                        )}
