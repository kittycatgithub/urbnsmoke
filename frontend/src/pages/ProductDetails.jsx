...
const ProductDetails = () => {
    ...
    return product && (
        <div className="max-w-6xl px-6 w-fit mx-auto my-16">
            <p>
                <Link to="/">Home</Link> /
                <Link to="/all-menu"> Menu</Link> /
                <Link to={`/menu/${product.category.toLowerCase()}`}> {product.category}</Link> /
                <span className="text-button"> {product.name}</span>
            </p>
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center mt-10 gap-4 text-base">
                        ...
                        <button onClick={ ()=> {addToCart(product._id);
                            navigate('/cart')
                        } } className="w-full py-3.5 cursor-pointer font-medium bg-primary-dull text-white hover:bg-green-700 transition" >
                            Buy now
                        </button>
                    </div>
                </div>
            ...
            <div className="flex flex-col items-center mt-20">
                <div className="flex flex-col items-center w-max">
                    ...
                    <div className="w-20 h-0.5 bg-button rounded-full mt-1"></div>
                </div>
        ...
        <button onClick={ ()=> {
            navigate('/all-menu');
            scrollTo(0,0)
        } } className="mx-auto cursor-pointer px-12 my-16 py-2.5 border rounded text-button hover:bg-button/10 transition">See More</button>
            </div>
        </div>
